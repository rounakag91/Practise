...
public class HourGlassMaxSum {
    ...
    private static final int width = 2; // hourglass width
    ...
    **/
    private static void hourglass(int i, int j){
        ...
        for(int k = j; k <= j + width; k++){
            tmp += matrix[i][k]; 
            tmp += matrix[i + width][k]; 
        }
        tmp += matrix[i + 1][j + 1];         // sum middle element
    ...
    public static void main(String[] args) {
        ...
        for(int i=0; i < _MAX - width; i++){
            for(int j=0; j < _MAX - width; j++){
                hourglass(i, j);
            }
        }
